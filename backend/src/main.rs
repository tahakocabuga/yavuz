...
use hyper::http::{Request, Response, Method, header};
...
async fn run() -> anyhow::Result<()> {
    ...
    #[openapi(
        paths(
            ...
            controllers::uav::get_uav_position,
            controllers::uav::update_uav_position
        ),
        components(
            schemas(models::mark::Mark, models::mark::NewMark, models::mark::UpdateMark, models::uav::UavPosition, models::uav::NewUavPosition)
        ),
    )]
    struct ApiDoc;
...
let store = Arc::new(controllers::uav::UavStore::default());
    let app = Router::new()
        ...
        .route("/uav", routing::get(controllers::uav::get_uav_position))
        .route("/uav/update", routing::post(controllers::uav::update_uav_position))
        .layer(cors)
